...
const CoachActivityManager = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (tempImage) {
                ...
                const fileName = `coach-event-${Date.now()}.jpg`;
                const { data: storageData, error: storageError } = await supabase.storage
                    .from('student-dashboard-image')
                    .upload(`coach-events/${fileName}`, croppedBlob);

                if (storageError) throw storageError;

                const { data: { publicUrl } } = supabase.storage
                    .from('student-dashboard-image')
                    .getPublicUrl(`coach-events/${fileName}`);
    ...
    const handleDelete = async (id) => {
        ...
        if (!window.confirm('📌 確定要刪除這項教練活動嗎？此操作無法還原。')) return;

        console.log('正在刪除活動，ID:', id);

        try {
            setIsLoading(true);
            // 使用 count: 'exact' 來檢查到底有沒有刪掉東西
            ...
            if (error) {
                ...
                alert(`❌ 教練活動刪除失敗 (狀態碼 ${status}): ${error.message}\n提示: ${error.hint || '無'}`);
                return;
            }

            // 如果 error 是 null，但 count 是 0，代表資料庫拒絕了這次操作 (通常是 RLS)
            if (count === 0) {
                alert('⚠️ 刪除指令已發送但「未成功刪除」。\n這通常是 Supabase 的 RLS 權限問題，請確認您已執行 SQL Editor 中的權限修復指令。');
                // 為了同步，重新抓取資料確保畫面顯示正確
                await fetchEvents();
                return;
            }
            ...
            <header className="page-header">
                <div className="header-top">
                    <div className="title-wrap">
                        <h2 className="page-title">教練活動管理</h2>
                ...
                {isEditing ? (
                    ...
                    <div className="activity-list">
                        {isLoading && events.length === 0 ? (
                            ...
                            <div className="empty-wrap">尚未建立教練活動</div>
                        ) : (
...
export default CoachActivityManager;
